using Rainfall.Api.Application.Common.Exceptions;
using Microsoft.Extensions.Logging;

namespace Rainfall.Api.Application.Common.Behaviours;
...
public class DbUpdateConcurrencyExceptionBehaviour<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : notnull
{
    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        catch (DbUpdateConcurrencyException ex)
        {
            ...
            _logger.LogError(ex, "Rainfall.Api Request: DbUpdateConcurrency Exception for Request {Name} {@Request}", requestName, request);
